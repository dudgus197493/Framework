...
public class MemberServiceImpl implements MemberService{
	...
	public Member login(Member inputMember) {
		...
		Member loginMember = dao.login(inputMember.getMemberEmail()); // AOP에서 트렌젝션 관리를할 것이라 dao에 Connection 객체를 인자로 줄 필요 x
		...
		if(loginMember != null) {	// 아이디 정상 입력
			if(bcrypt.matches(inputMember.getMemberPw(), loginMember.getMemberPw())) {
				// pass01!
				// asdkfhasdkfhalkdjfhasdkfhasdkjfh
	...
	// 코드기반 트랜잭션 처리 
	// 	- 지금까지 했던 service에서 코드로 직접 트랜잭션 처리
	/* ** 스프링에서 트랜잭션을 처리하는 방법
	 * - 선언적 트랜잭션 처리 
	 * 
	 ...
	 *   - 클래스, 메서드 위에 작성 가능
	 *   	- 클래스위에 작성 시 SELECT문에도 트랜잭션 처리를 시도 => 쓸대없는 동작 생김 => 메서드 위 작성 권장
	 *   - 조건 : AOP를 이용한 기술 -> Service Interface 필요
	 *            트랜잭션 매니저가 bean으로 등록되어야 함.
	 *   
