...
boardLike.addEventListener("click", e=>{
    ...
    if(e.target.classList.contains("fa-regular")) { // 빈하트인 경우
        $.ajax({
            ...
            success: result => {
                if(result > 0) {    // 성공
                    e.target.classList.remove("fa-regular");    // 빈하트 클래그 삭제
                    e.target.classList.add("fa-solid");         // 채워진 하트 클래스 추가
                    likeCount.innerText = Number(likeCount.innerText) + 1;  // 좋아요 1 증가
                } else {            // 실패
                    console.log("증가 실패");
                }
            },
        ...
        $.ajax({
            ...
            success: result=>{
                if(result > 0) {
                    ...
                    e.target.classList.add("fa-regular");           // 빈하트 클래그 추기
                    likeCount.innerText = Number(likeCount.innerText) - 1;  // 좋아요 1 감소
                } else {
                    console.log("감소 실패");
                }
            },
